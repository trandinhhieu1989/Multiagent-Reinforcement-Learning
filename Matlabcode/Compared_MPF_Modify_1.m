...
%% WE TRY TO COMPARE BETWEEN BMCU ALGORITHM AND TOTAL CONTENT POPULARITY ALGORITHM (CACHE FILES BASED ON CONTENT 
%% POPULARITY OF FILE AT ALL TIME PERIODS)
...
M = 4; % Number of samples creating each running times. In this paper, they defined T=F+1 as the total number of requests in time T
N = 500;%N: total number of time period T 
...
%%
    ...
    Down_Latency_UB = zeros(1,S_len); % Immediately content popularity
    Down_Latency_CP = zeros(1,S_len); % Total content popularity
    Number_Ser_D2D = zeros(1,S_len); % Initialize the number of file served by D2D or self-offloading
    Number_Ser_D2D_UB = zeros(1,S_len);% Immediately content popularity
    Number_Ser_D2D_CP = zeros(1,S_len);% Total content popularity
    %S_idx = 1:S_len
for S_idx = 1:S_len 
    %S_u = S_uVec(S_idx);
    %u_coor_matr = Topology(Radius,C,U);
...
%% TOTAL CONTENT POPULARITY ALGORITHM 
 % CALCULATING ADL AND CHR IN UB
    latency_BS_UE = s_f/R_BS_UE(u,:);
    id_req_CP = find (Temp_user_req_matrix(u,f)~=0);% Finding which file f is requested by user u
      if ~isempty(id_req_CP)
          if Cache_CP(u,f)==1 % If user u cache file f
             Number_Ser_D2D_CP(1,S_idx)  = Number_Ser_D2D_CP(1,S_idx)  + 1; % Increasing the number of self-offloading or D2D transmission
          else
...
% Finding the lowest latency from subset Nei_u_f to user u   
                if ~isempty(Nei_u_f_CP)
                    ...
                    Down_Latency_CP(1,S_idx)  = Down_Latency_CP(1,S_idx)  + latency_CP;
                    Number_Ser_D2D_CP(1,S_idx)  = Number_Ser_D2D_CP(1,S_idx)  + 1;
                else % No any neighbors of u cache file f, so transmission from BS
                    Down_Latency_CP(1,S_idx)  = Down_Latency_CP(1,S_idx)  + latency_BS_UE;                    
                end %if ~isempty(Nei_u_f_UB)
             else % transmission from BS
                 Down_Latency_CP(1,S_idx)  = Down_Latency_CP(1,S_idx)  + latency_BS_UE;
             end %if ~isempty(Nei_u_UB)
...
ADL(3,S_idx)  = Down_Latency_CP(1,S_idx)/sum(sum(User_req_matrix));
CHR(1,S_idx) = Number_Ser_D2D(1,S_idx)/sum(sum(User_req_matrix));
CHR(2,S_idx) = Number_Ser_D2D_UB(1,S_idx)/sum(sum(User_req_matrix));
CHR(3,S_idx) = Number_Ser_D2D_CP(1,S_idx)/sum(sum(User_req_matrix));
...
semilogy(S_u,CHR(3,:),'g^-'); grid on;hold on;
xlabel('Cache size of UEs')
ylabel('CHR')
toc
